...
class _DoctorViewScreenState extends State<DoctorViewScreen> {
  ...
  Map<String, dynamic> _computeStats() {
    ...
    return {
      'daysLogged': n, 'avgWater': tw / n, 'avgOxalate': to / n,
      'waterGoalDays': wg, 'oxalateGoalDays': og,
  ...
  Future<Uint8List> _buildPdf() async {
    ...
    // ── Bar chart widget builder ──
    pw.Widget barChart({
      required List<double> values,
      required double goal,
      required PdfColor barColor,
      required String unit,
      required bool goalIsMax, // true = water (want >= goal), false = oxalate (want <= goal)
    }) {
      if (values.isEmpty) {
          ...
          children: [
            ...
            pw.Positioned(
              ...
              child: pw.Text(
                ...
                style: pw.TextStyle(fontSize: 7, color: barColor, fontWeight: pw.FontWeight.bold),
              ),
            ),
            // Bars
            pw.Positioned(
                  ...
                  children: [
                    if (_userName.isNotEmpty)
                      pw.Text('Patient: $_userName',
                          style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold, color: pdfBlack)),
                ...
                pw.Expanded(
                  child: pw.Row(children: [
                    pw.Container(width: 10, height: 10,
                        decoration: const pw.BoxDecoration(color: pdfTeal, shape: pw.BoxShape.circle)),
                    pw.SizedBox(width: 6),
                    pw.Text('Water goal: ≥ ${_waterGoal.toStringAsFixed(0)} oz/day',
                        style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold, color: pdfTeal)),
                  ]),
                ),
                pw.Expanded(
                  child: pw.Row(children: [
                    pw.Container(width: 10, height: 10,
                        decoration: const pw.BoxDecoration(color: pdfRed, shape: pw.BoxShape.circle)),
                    pw.SizedBox(width: 6),
                    pw.Text('Oxalate limit: ≤ ${_oxGoal.toStringAsFixed(0)} mg/day',
                        style: pw.TextStyle(fontSize: 10, fontWeight: pw.FontWeight.bold, color: pdfRed)),
                  ]),
                ),
              ],
            ),
          ...
          child: Column(
            ...
            children: [
              ...
              _exportOption(
                ...
                title: 'PDF Report',
                subtitle: 'Charts, summary table & full daily log — best for printing or emailing',
                onTap: () {
              ...
              _exportOption(
                ...
                iconColor: _teal,
                title: 'Plain Text',
                subtitle: 'Simple text — great for texting or copying into a patient portal',
                onTap: () {
                  Navigator.pop(context);
                  _shareText();
                },
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  _buildLineCard(
                    title: 'Daily Water Intake (oz) — goal ${_waterGoal.toStringAsFixed(0)} oz',
                    color: _teal,
                    goal: _waterGoal,
                    valueSelector: (e) => (e['water_oz'] as num).toDouble(),
                  ),
                  const SizedBox(height: 16),
                  _buildLineCard(
                    title: 'Daily Oxalate Load (mg) — limit ${_oxGoal.toStringAsFixed(0)} mg',
                    color: _red,
                    goal: _oxGoal,
                    valueSelector: (e) => (e['oxalate_mg'] as num).toDouble(),
                  ),
                  const SizedBox(height: 16),
                  _buildExportCard(),
      ...
      child: ListView(
        ...
        children: _timeframes.map((tf) {
          ...
          return GestureDetector(
            onTap: () {
              setState(() { _daysBack = tf['days'] as int; _isLoading = true; });
              _loadData();
            },
            child: Container(
  ...
  Widget _buildHeaderNote() => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: const [
      Text('For your healthcare provider',
          style: TextStyle(color: _textMuted, fontSize: 13, fontWeight: FontWeight.w500)),
      SizedBox(height: 4),
      Text(
        'These charts show my daily water intake and oxalate load. '
            'Please review together with labs, imaging, and clinical history.',
      ...
      child: Column(
        ...
        children: [
          ...
          Row(children: [
            ...
            _statTile('Avg Oxalate', '${avgOxalate.toStringAsFixed(1)} mg', _red),
          ]),
          const SizedBox(height: 12),
          Row(children: [
            _statTile('Water Goal Met', '$waterPct%', _teal),
            _statTile('Oxalate Goal Met', '$oxalatePct%',
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(title,
              style: const TextStyle(
                  color: _textPri, fontWeight: FontWeight.bold, fontSize: 13)),
          const SizedBox(height: 12),
          SizedBox(
            height: 180,
  ...
  Widget _buildExportCard() => Container(
    padding: const EdgeInsets.all(16),
    decoration: BoxDecoration(
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('No history yet',
              style: TextStyle(
                  color: Color(0xFF2C2C2C), fontSize: 17, fontWeight: FontWeight.bold)),
          SizedBox(height: 8),
          Text(

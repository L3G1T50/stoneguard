...
//
// KidneyShieldPaywall — conversion-optimised paywall screen.
//
// • Dynamically fetches real pricing from RevenueCat Offerings
// • Loading overlay during transaction processing
// • Restore Purchases button
// • Graceful fallback UI if offerings cannot be loaded
// • Updates SubscriptionNotifier on successful purchase/restore
...
class _PaywallScreenState extends State<PaywallScreen> {
  ...
  // ---- State ----
  bool _isLoading = true;        // true while offerings are being fetched
  bool _isPurchasing = false;    // true while a purchase/restore is in-flight
  Offerings? _offerings;
  Package? _selectedPackage;     // the package the user will buy
  String? _fetchError;           // non-null if offerings failed to load
  ...
  Future<void> _loadOfferings() async {
    ...
    try {
      ...
      setState(() {
        ...
        // Pre-select the first package in the default offering.
        _selectedPackage =
      ...
      if (!mounted) return;
      setState(() {
        _fetchError = 'Could not load pricing. Please try again.';
        _isLoading = false;
      });
    }
  }
  ...
  Future<void> _purchaseSelected() async {
    ...
    try {
      ...
      if (!mounted) return;
      // Update the global notifier so gated widgets react immediately.
      await SubscriptionNotifier.of(context).refresh();
      if (status.isSubscribed) {
        _showSnackBar('Welcome to StoneGuard Plus! 🎉', isError: false);
        Navigator.pop(context, true);
      }
    } on RevenueCatServiceException catch (e) {
      if (!mounted) return;
      // Cancellations are silent — no snackbar needed.
      if (!e.isCancellation) {
  ...
  Future<void> _restorePurchases() async {
    ...
    try {
      ...
      await SubscriptionNotifier.of(context).refresh();
      if (status.isSubscribed) {
        _showSnackBar('Purchase restored successfully!', isError: false);
        Navigator.pop(context, true);
      } else {
        _showSnackBar(
  ...
  void _showSnackBar(String message, {required bool isError}) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        backgroundColor: isError ? Colors.red.shade700 : _tealDark,
        duration: const Duration(seconds: 4),
      ),
    );
  }

  /// Formats the RevenueCat price string for display.
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        ...
        // ── Full-screen loading overlay during purchase/restore ──
        if (_isPurchasing)
          Container(
  ...
  // ---------------------------------------------------------------------------
  // Paywall body (offerings loaded successfully)
  // ---------------------------------------------------------------------------
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          // ── PACKAGE SELECTOR (dynamic from RevenueCat) ──
          if (packages.isEmpty)
  ...
  // Dynamically-rendered package tiles from RevenueCat
  ...
  // Static fallback if no offerings are configured yet in RevenueCat dashboard

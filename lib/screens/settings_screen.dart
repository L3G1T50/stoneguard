...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _exportForDoctor() async {
    ...
    // Gather water logs
    final waterKeys =
        prefs.getKeys().where((k) => k.startsWith('water_')).toList()..sort();
    final waterRows = waterKeys.map((k) {
    ...
    // Gather oxalate logs
    final oxalateKeys =
        prefs.getKeys().where((k) => k.startsWith('oxalate_')).toList()
          ..sort();
    final oxalateRows = oxalateKeys.map((k) {
    ...
    // ─── PDF COLORS ───
    const teal = PdfColor.fromInt(0xFF01696F);
    const tealLight = PdfColor.fromInt(0xFFE0F2F1);
    const textDark = PdfColor.fromInt(0xFF1A2E35);
    ...
    pw.Widget _dataTable(List<String> headers, List<List<String>> rows) {
      if (rows.isEmpty) {
        return pw.Container(
          ...
          child: pw.Text('No data recorded yet.',
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
            ...
            child: pw.Row(
              ...
              children: [
                pw.Column(
                  ...
                  children: [
                    pw.Text('StoneGuard — Doctor Report',
                        style: pw.TextStyle(
                            fontSize: 16,
                            fontWeight: pw.FontWeight.bold,
                            color: PdfColors.white)),
            ...
            child: pw.Row(
              children: [
                ...
                pw.Container(
                  ...
                  color: divider,
                  margin:
                      const pw.EdgeInsets.symmetric(horizontal: 16),
                ),
                pw.Expanded(
                  child: pw.Column(
                    crossAxisAlignment: pw.CrossAxisAlignment.start,
                    children: [
                      pw.Text('Water Goal',
                          style: pw.TextStyle(
                              ...
                              fontWeight: pw.FontWeight.bold)),
                      pw.SizedBox(height: 3),
                      pw.Text('${_waterGoal.toInt()} oz / day',
                          style: pw.TextStyle(
                              fontSize: 12,
                              color: textDark)),
                    ],
                ...
                pw.Expanded(
                  child: pw.Column(
                    crossAxisAlignment: pw.CrossAxisAlignment.start,
                    children: [
                      pw.Text('Oxalate Limit',
                          style: pw.TextStyle(
                              fontSize: 9,
                              color: textGrey,
                              fontWeight: pw.FontWeight.bold)),
                      pw.SizedBox(height: 3),
                      pw.Text('${_oxalateGoal.toInt()} mg / day',
                          style: pw.TextStyle(
                              fontSize: 12,
                              color: textDark)),
                    ],
          ...
          // ── WATER LOG TABLE ──
          _sectionHeader('Water Intake Log'),
          _dataTable(['Date', 'Amount'], waterRows),

          // ── OXALATE LOG TABLE ──
          _sectionHeader('Oxalate Intake Log'),
          _dataTable(['Date', 'Amount'], oxalateRows),
          ...
          pw.Container(
            ...
            child: pw.Text(
              'DISCLAIMER: This report is generated by StoneGuard for informational purposes only. '
              'It is not a substitute for professional medical advice, diagnosis, or treatment. '
              'Always consult your physician or a qualified healthcare provider regarding your '
              'kidney stone condition and dietary needs.',
              style: pw.TextStyle(
                  fontSize: 8,
        ...
        footer: (context) => pw.Container(
          ...
          child: pw.Text(
            'StoneGuard • stoneguard.app • Page ${context.pageNumber} of ${context.pagesCount}',
    ...
    await Printing.sharePdf(
      bytes: bytes,
      filename: 'StoneGuard_Report_$dateStr.pdf',
    );
  }
              ...
              children: [
                Text(
                  ...
                  style: AppTextStyles.itemTitle.copyWith(
                    color:
                        _isPremium ? Colors.white : AppColors.textPrimary,
                  ),
                ),
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          padding:
              AppSpacing.pagePadding.add(const EdgeInsets.only(bottom: 32)),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              // ── TITLE ──
              ...
              AppCard(
                child: _row(
                  ...
                  'Export for Doctor',
                  _isPremium
                      ? 'Generates a PDF of your water & oxalate logs'
              ...
              Center(
                child:
                    Text('StoneGuard v1.0.0', style: AppTextStyles.micro),
              ),
              const SizedBox(height: 16),
            ],
          ),
